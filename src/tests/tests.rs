...
fn test_crud_user() {
    ...
    let mut response = client.post("/api/users")
        .body(serde_json::to_string(&UserCreate {
            id: None,
    ...
    let created_body: User = serde_json::from_str(response.body_string().unwrap().as_str()).unwrap();

    // Read user
    let mut response = client.get(format!("/api/users/{}", created_body.id))
        .header(ContentType::JSON)
        .dispatch();

    let fetched_body: User = serde_json::from_str(response.body_string().unwrap().as_str()).unwrap();
    ...
    let mut response = client.patch("/api/users")
        .body(serde_json::to_string(&UserUpdate{
            id: fetched_body.id,
    ...
    let updated_body: User = serde_json::from_str(response.body_string().unwrap().as_str()).unwrap();
    ...
    // Delete user
    let mut response = client.delete(format!("/api/users/{}", updated_body.id))
        .header(ContentType::JSON)
        .dispatch();

    let deleted_body: User = serde_json::from_str(response.body_string().unwrap().as_str()).unwrap();
    assert_eq!(response.status(), Status::Ok);
